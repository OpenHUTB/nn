...
从课程作业的角度看，这类项目的重点不只是让程序在一段视频上显示检测框，而是要说明系统为什么这样判断、判断依据来自哪里、结果能否被复查。自动驾驶危险判断属于典型的多因素问题，单独依赖某个阈值虽然容易实现，但很难解释复杂场景中的风险变化。本次项目正是从这个问题出发，把视觉检测结果进一步转化为可解释的风险指标。
...
四个层面的设计让项目形成了比较完整的工程闭环：输入端来自 CARLA 或视频，处理中间有车道检测、目标检测和风险评分，输出端有可视化结果和指标文件。这样汇报时可以从“为什么要改”讲到“怎么改”，再讲到“改完之后如何验证”，内容会比单纯介绍一个函数更完整。

在实现思路上，本项目尽量保留原系统的已有功能，把新增逻辑放在风险评估和报告导出部分。这样做的好处是改动范围比较清晰，不会把车道检测、目标检测、可视化展示全部推倒重写，也更符合在已有课程项目上做增量优化的要求。
...
在汇报时，这一部分可以作为重点展示。动图能够直观说明系统不是突然报警，而是随着风险因素变化逐步提高危险等级。老师看到这一部分时，也能更容易理解本项目和简单目标检测演示之间的区别。
...
如果只输出最终的 `BRAKE!`，观众很难知道系统为什么触发制动。现在页面中展示了风险分数、TTC 和车道关系，能够说明危险判断是由多个因素共同决定的，这会让汇报内容更有说服力。
...
报告输出模块解决的是“运行结果如何保存”的问题。很多演示程序只能在窗口中看一遍效果，程序关闭后就很难复查。本项目把运行结果保存为多个文件，使得一次实验可以被反复查看，也能在 PR 说明、课程报告和答辩展示中复用。
...
如果上台汇报时间比较充足，可以把测试过程讲成一条流水线：CARLA 生成图像帧，程序逐帧读取，车道和目标检测模块提取关键信息，风险模块计算评分，报告模块保存结果。这样老师可以清楚看到项目从输入到输出的完整路径。
...
从课程要求上看，本项目使用了 CARLA 仿真器数据，具备运行效果图、动态变化展示和可复查的输出结果。相比仅提交代码，配套文档和页面能够更完整地说明项目价值，也便于老师理解工程量。
...
如果将本项目作为课堂展示内容，可以按照“原始问题、改进方案、模块结构、核心算法、CARLA 数据、运行效果、报告输出、工程意义”这个顺序讲解。这样既能展示项目做了什么，也能说明为什么这样做，最后再用图片和动图展示实际效果，整体汇报会比较完整。

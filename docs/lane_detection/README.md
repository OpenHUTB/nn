...
> 完整课题提交（PR1–PR10）：基础检测 → HSV 优化 → 透视变换 → 视频处理 → 曲率计算 → 偏离预警 → 快速搜索 → 方法对比 → 文档总结

基于 OpenCV 的 Carla 场景车道线检测模块，分步完成预处理、边缘检测、霍夫直线检测与 HSV 多车道拟合。

**作者**：ultra223  
**课题进度**：10/10（步骤1 → 步骤8 全部完成，详见下方总结）
...
| 维度 | 步骤1 基础版 | 步骤2 HSV | 步骤3 高级版 | 步骤4-8 完整版 |
|:---|:---|:---|:---|:---|
| **车道线类型** | 直线 | 直线 | 曲线 + 直线 | 曲线 + 直线 |
| **预处理** | 灰度 + Canny | HSV 颜色阈值 | HSV + Sobel 联合 | HSV + Sobel（最优） |
| **检测方法** | 霍夫变换 | 滑动窗口 + 线性拟合 | 滑动窗口 + 二次多项式 | 带搜索 + 二次多项式 |
| **弯道支持** | 否 | 否 | 是 | 是 |
| **视频支持** | 否 | 否 | 否 | 是（EMA 平滑） |
| **曲率/偏移** | 否 | 否 | 是 | 是（实时） |
| **偏离预警** | 否 | 否 | 否 | 是（三级） |
| **实时性能** | — | — | — | ~30ms/帧 |
| **适用场景** | 简单直道演示 | 多车道直道 | 弯道单帧分析 | 完整驾驶辅助 |

## 适用场景

| 场景 | 推荐模式 | 说明 |
|:---|:---|:---|
| 简单直道演示 | `basic` | 快速理解 Canny + 霍夫原理 |
| 多车道道路 | `hsv` | 区分黄/白线，划分左右车道 |
| 弯道分析 | `advanced` | 透视变换 + 多项式拟合 |
| 行车视频分析 | `video` | 完整管线：检测 → 曲率 → 偏移 → 预警 |
| 预处理调优 | `compare` | 对比三种方法，选择最优策略 |
...
本课题从零开始，逐步构建了一个完整的车道线检测系统，涵盖从简单图像处理到复杂驾驶辅助的完整技术栈：

1. **检测能力演进**：从 Canny 边缘 + 霍夫直线 → HSV 颜色空间 → 透视变换 + 多项式拟合，逐步支持弯道检测
2. **驾驶信息提取**：从纯像素级检测 → 曲率半径计算 → 车辆偏移 → 偏离预警，构建完整的驾驶辅助信息链
3. **工程优化**：EMA 帧间平滑消除抖动、带搜索大幅提升视频处理速度（3.8x~16.5x）、预处理对比评估选择最优策略
4. **模块化设计**：所有功能通过 `main.py --mode` 统一入口，配置集中在 `config.py`，各步骤独立模块可单独测试
...
| 视频处理性能 | ~30ms/帧（快速搜索） |
| 带搜索成功率 | > 99.7%（回退率 < 0.3%） |
| 最优预处理 | HSV + Sobel 联合（像素数 20896，R² > 0.78） |
| 预警等级 | 3 级（安全/注意/危险） |

...
**作者**：ultra223  
**课题进度**：10/10（步骤1 基础检测 + 步骤2 HSV 优化 + 步骤3 透视变换+多项式拟合 + 步骤4 视频处理 + 步骤5 曲率与偏移计算 + 步骤6 车道偏离预警 + 步骤7 快速搜索优化 + 步骤8 预处理对比评估 + 步骤9 方法对比与适用场景 + 步骤10 课题总结）
...
| 步骤 | 方法 | 支持弯道 | 支持视频 | 实时预警 | 适用场景 |
|:---|:---|:---|:---|:---|:---|
| 1 | Canny + 霍夫 | 否（直线） | 否 | 否 | 简单直道场景验证 |
| 2 | HSV 颜色阈值 | 否（直线） | 否 | 否 | 黄/白车道线清晰的场景 |
| 3 | 透视变换 + 多项式拟合 | 是 | 否 | 否 | 弯道检测、曲线拟合 |
| 4 | 视频处理 + EMA 平滑 | 是 | 是 | 否 | 连续视频帧去抖动 |
| 5 | 曲率半径 + 偏移计算 | 是 | 是 | 否 | 量化驾驶状态 |
| 6 | 车道偏离预警 | 是 | 是 | 是 | 实时危险提示 |
| 7 | 快速搜索优化 | 是 | 是 | 是 | 高速视频处理 |
| 8 | 预处理对比评估 | 是 | 是 | 是 | 选择最优预处理策略 |

**推荐配置**：HSV + Sobel 联合预处理 + 多项式拟合 + 视频 EMA 平滑 + 三级预警。

## 步骤10：课题总结

**问题定义**：基于 OpenCV 实现 Carla 场景下的车道线检测与偏离预警系统。

**技术路线**：颜色/梯度预处理 → 鸟瞰图透视变换 → 滑动窗口/带搜索 → 二次多项式拟合 → 帧间 EMA 平滑 → 曲率与偏移计算 → 三级偏离预警。
...
|:---|:---|
| 核心模块 | 8 个 `.py` 文件 |
| 运行模式 | basic / hsv / advanced / video / compare |
| 视频处理速度 | 约 28-30 ms/帧（启用快速搜索） |
| 速度提升（快速搜索 vs 滑动窗口） | 3.8x ~ 16.5x |
| 视频检测稳定性（317+364 帧） | 回退率 < 0.15% |
| 多项式拟合 R²（最优方法） | 左 0.78 / 右 0.90 |
| 预警等级 | 安全 / 注意 / 危险 三级 |

**关键改进**

- 从霍夫直线到多项式曲线：支持弯道检测
- 引入透视变换：消除透视畸变，鸟瞰图视角更准确
- 视频模式 + EMA 平滑：减少帧间抖动
- 快速搜索：复用前一帧结果，速度提升 3-16 倍
- 三级预警系统：将检测结果转化为可行动的驾驶提示

**局限性**

- 严重遮挡或车道线磨损时检测可能失败
- 仅支持黄/白车道线，需扩展其他颜色
- 未结合深度学习，复杂场景需 CNN 方法补充

...
3.  **填写连接信息**：
    ...
    *   **URL**：粘贴以下地址，并务必将 `你的密钥` 替换为你在第一章获取的真实密钥。
    ...
    ```
4.  **加载图层**：点击“确定”后，双击此新建连接，天地图卫星影像将加载到主地图窗口。
...
### 3.2 地理配准（核心步骤）
导入的底图必须被赋予真实世界坐标，才能作为精确参考。

1.  点击顶部菜单：**World → World Settings**。
2.  在World Settings工具中：
    *   **设置坐标系**：通常选择 **`WGS84`**（EPSG:4326）。
    *   **设置原点坐标**：输入底图**左下角（西南角）** 的精确经纬度。此坐标可在QGIS导出前，通过将鼠标移至该点，从软件底部状态栏读取。
3.  确认设置后，底图将自动对齐到正确的地理位置。

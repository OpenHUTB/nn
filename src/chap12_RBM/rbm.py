...
class RBM:
    ...
    def __init__(self, n_hidden=2, n_observe=784):
        ...
        init_std = np.sqrt(2.0 / (self.n_observe + self.n_hidden))  # Xavier 初始化标准差
    ...
    def _sigmoid(self, x):
        """Sigmoid 激活函数，用于将输入映射到概率空间"""
        return 1.0 / (1 + np.exp(-x))

    def _sample_binary(self, probs):
        """伯努利采样：根据给定概率生成 0 或 1（用于模拟神经元激活）"""
        return np.random.binomial(1, probs)

    def train(self, data):
        """
        使用 Contrastive Divergence 算法对模型进行训练
        参数说明：
        ...
        for epoch in range(epochs):
            ...
            for i in range(0, n_samples, batch_size):  # 获取当前批次的数据
                ...
                # 正相传播：从 v0 计算隐藏层激活概率
                h0_prob = self._sigmoid(np.dot(v0, self.W) + self.b_h)  # 通过输入层向量 v0 与权重矩阵 self.W 的点积，再加上隐藏层偏置 self.b_h
    ...
    def sample(self):
        """从训练好的模型中采样生成新数据（Gibbs 采样）"""
        # 初始化可见层：使用伯努利分布随机生成二值向量（每个像素有 50% 概率为 1）
        # n_observe 是可见层神经元数量（28x28=784）
        v = np.random.binomial(1, 0.5, self.n_observe)

        # 进行 1000 次 Gibbs 采样迭代，以逐步趋近真实数据分布
        for _ in range(1000):
            # 基于当前的可见层 v，计算隐藏层神经元被激活的概率（前向传播）

...
# ===================== 新增：车道偏航角计算（纯计算功能） =====================
def calculate_yaw_angle(left_fit, right_fit, img_shape):
    h, w = img_shape[:2]
    ...
    # 左右线底部坐标
    xl1 = left_fit[0]*y1**2 + left_fit[1]*y1 + left_fit[2]
    xr1 = right_fit[0]*y1**2 + right_fit[1]*y1 + right_fit[2]
    # 左右线上部坐标
    xl2 = left_fit[0]*y2**2 + left_fit[1]*y2 + left_fit[2]
    xr2 = right_fit[0]*y2**2 + right_fit[1]*y2 + right_fit[2]
...
def lane_detection_pipeline(img):
    ...
    # ===================== 显示在图片上 =====================
    yaw_angle = calculate_yaw_angle(left_fit, right_fit, img.shape)
    cv2.putText(result, f"Yaw Angle: {yaw_angle}",
...
def main():
    print("="*50)
    print("      Lane Detection + Yaw Angle")
    print("="*50)

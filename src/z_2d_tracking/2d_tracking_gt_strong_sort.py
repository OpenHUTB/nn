...
def ensure_vehicle_alive(vehicle, world, bp_lib, spawn_points):
    """确保玩家车辆存活，如果被销毁则重新生成"""
    if vehicle is None or not vehicle.is_alive:
        ...
        if vehicle is None:
            print("Warning: Failed to respawn vehicle")
        ...
        return vehicle, camera, camera_bp
...
while True:
    try:
        ...
        # 检查并确保玩家车辆存活
        vehicle, new_camera, new_camera_bp = ensure_vehicle_alive(
            vehicle, world, bp_lib, spawn_points)
        if vehicle is None:
            break
        if new_camera is not None:
            camera = new_camera
            camera_bp = new_camera_bp
            # 重新计算相机参数
            image_w = camera_bp.get_attribute("image_size_x").as_int()
            image_h = camera_bp.get_attribute("image_size_y").as_int()
            fov = camera_bp.get_attribute("fov").as_float()
        ...
        for npc in world.get_actors().filter('*vehicle*'):
            ...
            # Filter out the ego vehicle and check if NPC is alive
        ...
        if len(bboxes) > 0:
            ...
            # 使用实际检测到的标签，而不是全部标记为 car
            detection_labels = np.array(labels) if len(labels) == len(bboxes) else np.array([2] * len(bboxes))

            image = draw_bounding_boxes(
                image, bboxes, detection_labels, COCO_CLASS_NAMES, ids)

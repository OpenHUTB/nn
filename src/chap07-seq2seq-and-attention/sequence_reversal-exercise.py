...
# 定义自定义序列到序列(Seq2Seq)模型，继承自Keras的Model类
# Seq2Seq模型通常用于处理变长输入到变长输出的任务，如机器翻译、对话生成等
# 包含编码器(Encoder)和解码器(Decoder)两个核心组件，通过上下文向量连接
...
def compute_loss(logits, labels):
    ...
    # 计算平均损失
    # 对损失张量进行降维求平均，得到一个标量值
    # 常用于批量训练中，将每个样本的损失值汇总为一个整体损失
    # 例如：输入形状为[batch_size]的损失张量，输出为单个标量值

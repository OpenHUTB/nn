...
class Softmax:
    ...
    def backward(self, grad_y):
        ...
        s = self.mem['out']
        sisj = np.matmul(
            np.expand_dims(s, axis=2), np.expand_dims(s, axis=1)
        )
...
def prepare_data():
    ...
    train_label[np.arange(train_data[0].shape[0]),
                np.array(train_data[1])] = 1.
    test_label[np.arange(test_data[0].shape[0]),
               np.array(test_data[1])] = 1.

...
### 安装 MuJoCo 引擎
1. 前往 MuJoCo 官网 下载适合你操作系统的 MuJoCo 版本
2. 解压并放置在 `~/.mujoco/mujoco-2.3.7` (Linux/macOS) 或 `C:\Users\YourUser\mujoco-2.3.7` (Windows)
3. (Linux) 添加环境变量：
   ```bash
   echo 'export LD_LIBRARY_PATH=$LD_LIBRARY_PATH:$HOME/.mujoco/mujoco-2.3.7/bin' >> ~/.bashrc
   ```

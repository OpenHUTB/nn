...
`src/box` 包含若干用于仿真、感知与强化学习实验的环境与辅助脚本。主要面向基于 Gymnasium/MuJoCo 的任务、轻量级可视化示例以及调试/测试工具。（Gymnasium：一个操作台，是网上的一个强化学习的开源项目，给你提供一套通用的指令，可以完成一些基础操作。并提供给你进一步写指令代码的框架。MuJoCo：一个物理仿真平台，计算你的指令在现实中执行后可能产生什么结果。在这个项目中是一起用，如果只是写一些小例子，可以只用Gymnasium）
本目录旨在提供：

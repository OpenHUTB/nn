...
# Define the path to the CARLA file
...
# 检测红绿灯状态
def check_red_light(world, vehicle):
...
def draw_violation_info(img):
    ...
    if violation_info["red_light"]:
        cv2.putText(img, "VIOLATION: RED LIGHT!", (20, 150),
                    cv2.FONT_HERSHEY_SIMPLEX, 2, (0, 0, 255), 4)
    if violation_info["ignore_sign"]:
        cv2.putText(img, "VIOLATION: IGNORE SIGN!", (20, 200),
                    cv2.FONT_HERSHEY_SIMPLEX, 2, (0, 0, 255), 4)
...
try:
    while True:
        ...
        # === 核心：违章检测 ===
        detect_violations(world, vehicle)

...
def prepare_mnist_features_and_labels(x, y):
    ...
    Returns:
        ...
        y: 转换为整型的标签。
    """
    # 将输入图像数据 x 的数据类型转换为 tf.float32（浮点型）
    # 然后除以 255.0，将像素值从 [0, 255] 归一化到 [0.0, 1.0] 范围内
    # 这是训练神经网络时常见的预处理步骤，有助于加快训练速度和提升模型性能
    x = tf.cast(x, tf.float32) / 255.0
    # 将标签 y 的数据类型转换为 tf.int64（长整型）
    # 通常在使用交叉熵损失函数
    y = tf.cast(y, tf.int64)
    # 返回处理后的输入特征 x 和对应的标签 y
    return x, y

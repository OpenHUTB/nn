...
def main():
    ...
    # 配置文件
    config = {
        'model_path': "D:\\vscode\\vscode project\\mujoco_learning\\RobotH.xml",
        'data_paths': {
            'walk': "D:\\zha\\loco-mujoco\\cache\\DEFAULT\\mocap\\UnitreeH1\\walk.npz",
            'squat': "D:\\zha\\loco-mujoco\\cache\\DEFAULT\\mocap\\UnitreeH1\\squat.npz"
        },
...
def quick_demo():
    ...
    try:
        if choice == '1':
            # 加载专家数据
            walk_data = np.load("D:\\zha\\loco-mujoco\\cache\\DEFAULT\\mocap\\UnitreeH1\\walk.npz", allow_pickle=True)
            ...
            if os.path.exists(model_path):
                # 创建环境获取维度信息
                env = UnitreeEnv("D:\\vscode\\vscode project\\mujoco_learning\\RobotH.xml")

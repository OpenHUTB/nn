...
# 恒等基函数（Identity Basis Function）的实现
def identity_basis(x):
    """
    将输入x转换为适合线性回归的格式
    在x的最后一个维度上增加一个维度，将其转换为二维数组
    用于适配线性回归的矩阵运算格式
    """
    # 通过 np.expand_dims，将 x 转换为列向量的形式，形状变为 (len(x), 1)
    return np.expand_dims(x, axis=1)

# 多项式基函数（Multinomial Basis Function）
...
def gaussian_basis(x, feature_num=10):
    ...
    # 计算每个输入 x 对所有中心的响应
    # 输出 shape (N, feature_num)，表示N个样本在feature_num个基函数上的响应值
...
def main(x_train, y_train, use_gradient_descent=False):
    ...
    # 生成偏置项和特征矩阵
    phi0 = np.expand_dims(np.ones_like(x_train), axis=1)  # 构造偏置项1
...
if __name__ == "__main__":
    ...
    plt.plot(x_test, y_test, "k-", linewidth=2)  # 黑色实线为测试集真实值
    plt.plot(x_test, y_test_pred, "b--", linewidth=2)  # 蓝色虚线为测试集预测值
    plt.xlabel("x")  # 设置x轴的标签
    plt.ylabel("y")  # 设置y轴的标签
    plt.title("线性回归模型预测结果")  # 设置图表标题
    plt.legend(["训练数据", "测试数据真实值", "测试数据预测值"])  # 添加图例，表示每条线的含义
    plt.grid(True, linestyle='--', alpha=0.7)  # 添加网格线，增强可读性
    plt.tight_layout()  # 调整布局，使图表更加美观
    plt.show()  # 显示图表

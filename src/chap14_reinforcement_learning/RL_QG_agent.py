...
import os
# 数值计算库 - Python 科学计算的核心库
import numpy as np
# 深度学习框架 - Google 开发的开源机器学习平台
...
class RL_QG_agent:
    ...
    def __init__(self):
        ...
        # self.model_dir 用于存储模型文件的目录路径。os.path.dirname(os.path.abspath(__file__)) 获取当前脚本文件的绝对路径，并提取其所在的目录
        # 用于初始化与模型保存、TensorFlow 会话以及输入和输出张量相关的属性
        os.makedirs(self.model_dir, exist_ok=True)  # 创建模型保存目录（如果目录不存在则自动创建）
        self.sess = None  # TensorFlow 会话对象初始化占位
        self.saver = None  # TensorFlow 模型保存器初始化占位
        self.input_states = None  # 神经网络输入占位符初始化占位
        self.Q_values = None  # 神经网络输出的 Q 值初始化占位
    ...
    def init_model(self):
        ...
        # 定义输入状态，假设为 8x8 棋盘，3 个通道（如当前玩家棋子、对手棋子、可行位置）
    ...
    def place(self, state, enables):
        ...
        """
        # 状态预处理
        state_input = np.array(state).reshape(1, 8, 8, 3).astype(np.float32)  # 转换为 (1, 8, 8, 3) 形状

...
class CarlaEnv:
    ...
    def _set_env(self):
        ...
        for i in range(self.config['car_num']):
            car = self.world.spawn_actor(random.choice(cars), self.spawn_points[i])
    ...
    def reset(self):
        ...
        frame_index = self.world.tick()
        self.update_spectator()
        assert len(self.world.get_actors().filter('*vehicle*')) == (self.config['car_num'] + 1), "set env wrong"
    ...
    def reward_sac(self, collision):
        return -200 if collision != 0 else 1

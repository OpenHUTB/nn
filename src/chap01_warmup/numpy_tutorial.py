...
b = a[0:2, 1:3]  # 0:2表示取第 0 行（包含）到第 2 行（不包含），即实际取第 0 行和第 1 行；1:3表示取第 1 列（包含）到第 3 列（不包含），即实际取第 1 列和第 2 列 
print("(1)输出b\n", b)
...
plt.plot(x, y, label="y = x^2", color="blue")  # 绘制蓝色曲线，并设置图例标签为y = x^2

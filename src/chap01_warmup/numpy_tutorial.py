...
print("print(np.sum(x)):", np.sum(x)) #计算整个数组中所有元素的总和
print("print(np.sum(x, axis=0))", np.sum(x, axis=0)) #按列求和。axis=0 表示沿着列的方向进行操作。
print("print(np.sum(x, axis=1))", np.sum(x, axis=1)) #按行求和。axis=1 表示沿着行的方向进行操作。

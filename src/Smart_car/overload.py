...
class UnmannedVehicleOverloadWarningSystem:
    """无人车超载预警系统
    功能：实时监测无人车载重状态，根据载重占比划分预警等级，输出可视化预警信息，严重超载时紧急停止系统
    """
    def __init__(self, max_load=1000):
        # 初始化系统核心参数
        self.max_load = max_load  # 车辆最大载重（单位：kg，可根据实际车型自定义配置）
        self.current_load = 0     # 当前实时载重，初始值为0
        self.warning_level = 0    # 预警等级：0-正常，1-轻度过载，2-中度过载，3-严重超载

    def simulate_weight_collection(self):
        """模拟无人车重量采集（模拟传感器数据，可替换为真实硬件接口）
        逻辑：在当前载重基础上随机小幅波动，模拟乘客上下车或货物装卸的载重变化
        返回：更新后的当前载重值
        """
        # 模拟载重变化：每次波动范围为-50kg（卸载）到100kg（加载）
        weight_change = random.randint(-50, 100)
        # 更新当前载重，确保载重不会为负数（最低为0）
    ...
    def judge_overload(self):
        """超载判断逻辑，根据载重占比划分预警等级
        计算规则：载重占比 = 当前载重 / 最大载重
        返回值：(预警状态名称, 预警颜色标识, 处理建议描述)
        """
        load_ratio = self.current_load / self.max_load  # 计算载重占比，用于等级判定

        # 正常状态：载重≤80%最大载重，无需干预
        if load_ratio <= 0.8:
            self.warning_level = 0
            return "正常", "绿色", "当前载重未超出安全范围，无需处理"
        # 轻度过载预警：80%<载重≤95%，接近上限需停止加载
    ...
    def display_warning(self, status, color, desc):
        ...
        参数：
            status: 预警状态名称（如"正常"、"严重超载警报"）
            color: 预警颜色标识（如"绿色"、"红色"）
            desc: 具体处理建议描述
        """
        print("=" * 50)
    ...
    def run(self, monitor_times=10):
        """运行系统，持续循环监测载重并输出预警信息
        参数：
            monitor_times: 预设监测次数，默认10次，可根据需求修改
        流程：采集载重→判断等级→显示预警→异常停止（严重超载）→间隔等待
        """
        print("无人车超载预警系统已启动...")
        ...
        for i in range(monitor_times):
            ...
            # 4. 严重超载（等级3）时，紧急停止系统运行
            if self.warning_level == 3:
                print("❗❗❗ 检测到严重超载，系统紧急停止运行！ ❗❗")
                break
            # 5. 间隔2秒进行下一次监测，模拟实时周期性检测
            time.sleep(2)

if __name__ == "__main__":
    # 初始化系统：设置无人车最大载重为1000kg（可根据车型/场景需求修改该参数）
    overload_system = UnmannedVehicleOverloadWarningSystem(max_load=1000)
    # 运行系统，持续监测10次（可修改monitor_times参数调整监测次数）

...
# 导入功能模块
from utils.math_utils import get_gravity_orientation, pd_control #数学工具
from utils.gamepad_utils import (init_gamepad, update_cmd_from_gamepad, joystick,get_current_mode)#手柄输入处理
from utils.mode_utils import mode_names #扰动力施加
...
if __name__ == "__main__":
    ...
    config_file = args.config_file #使用argparse解析命令参数
    with open(f"configs/{config_file}", "r") as f:
        ...
        cmd = np.array(config["cmd_init"], dtype=np.float32)#加载yaml中的参数
    ...
    counter = 0 #初始化动作、目标关节角度、观测值和计数器
    print("xml_path = ", xml_path)
    # Load robot model
    m = mujoco.MjModel.from_xml_path(xml_path)
    d = mujoco.MjData(m)
    m.opt.timestep = simulation_dt#加载机器人模型，设置仿真时间步长。
    
    # 保存初始状态用于reset
    initial_qpos = d.qpos.copy()
    initial_qvel = d.qvel.copy()
    initial_cmd = cmd.copy()  # 保存初始速度（位置，速度，控制命令）

    # load policy
    policy = torch.jit.load(policy_path)#加载PyTorch策略模型
    ...
    with mujoco.viewer.launch_passive(m, d) as viewer:#启动仿真循环

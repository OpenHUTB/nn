        ...
        with viewer.launch_passive(model, data) as v:
            while v.is_running() and (not use_ros or not rospy.is_shutdown()):
                ...
                if use_ros and ros_publishers is not None:
                    ...
                    joint_msg.position = data.qpos[:njnt].tolist()  # 关节位置（前njnt个自由度）
                    joint_msg.velocity = data.qvel[:njnt].tolist()  # 关节速度
                    joint_state_pub.publish(joint_msg)

                    # 2. 发布基座姿态（假设根关节是自由关节，取前7个自由度：x,y,z,qx,qy,qz,qw）
